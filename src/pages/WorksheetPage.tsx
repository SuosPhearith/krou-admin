...
const WorksheetPage = () => {
  ...
  // បើក Modal
  const openModal = (worksheet: Worksheet | null = null) => {
    setIsModalOpen(true);
    setIsEdit(!!worksheet);
  ...
  const handleFormSubmit = async (values: Omit<Worksheet, "id">) => {
    try {
      if (isEdit && currentWorksheet) {
        await axios.put(
          `${import.meta.env.VITE_APP_API_URL}/api/worksheets/update/${
            currentWorksheet.id
          }`,
          {
            ...values,
  ...
  // ទាញយក Worksheet នៅពេលទំព័របើកឡើង
  useEffect(() => {
    fetchWorksheets();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page]);
  ...
  const columns: ColumnsType<Worksheet> = [
    {
      title: "រូបភាពក្រប",
      dataIndex: "cover_uri",
      key: "cover_uri",
