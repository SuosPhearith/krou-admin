...
interface WorksheetDocument {
  ...
  title: string;
  file_uri: string;
  worksheets_id: number;
  status: boolean;
}

const WorksheetDocPage = () => {
  // State management
  const { id } = useParams<{ id: string }>();
  const [documents, setDocuments] = useState<WorksheetDocument[]>([]);
  ...
  // Fetch worksheet documents with pagination and search
  const fetchDocuments = async (searchValue: string = search) => {
    setLoading(true);
    try {
      const response = await axios.get<{
        data: WorksheetDocument[];
        total: number;
        current_page: number;
      }>(
        `${
  ...
  const handleFormSubmit = async (values: Omit<WorksheetDocument, "id">) => {
    try {
      if (!fileUri) {
  ...
  const columns: ColumnsType<WorksheetDocument> = [
    {
      title: "ចំណងជើង",
      dataIndex: "title",
      key: "title",
...
export default WorksheetDocPage;
